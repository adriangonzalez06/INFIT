...
export default function CreateDishMenu({ route }) {
        ...
        <View style={[styles.dishContainer]}>
          ...
          <View style={{ flex: 1 }}>
            <View key={ingredient.id} >
              <Text style={styles.title_2}>{ingredient.name}</Text>
  ...
  const saveDish = async () => {
    try {
      ...
      setDishName('');
      setDishDescription('');
      setIngredientsWithGrams([]);

      Alert.alert('¡Éxito!', 'El plato se ha guardado correctamente y está disponible para añadir a tu dieta.');
      
      // Volver atrás después de guardar
      navigation.goBack();
    } catch (err) {
      console.error('❌ Error saving dish:', err);
      Alert.alert('Error', err.message || 'No se pudo guardar el plato. Verifica la conexión al servidor.');
    }
  };
